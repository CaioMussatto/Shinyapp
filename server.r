server <- function(input, output, session) {
  ...
  output$heatmap <- renderPlot({
    ...
    nomes <- HeatmapAnnotation(
      df = as.data.frame(morpheus_data["Subtype"]), which = "row", annotation_width = unit(c(1, 3), "cm"),
    ...
    Heatmap(morpheus_data,
      right_annotation = nomes, rect_gp = gpar(col = "white", lwd = 0.8),
  ...
  output$enrichr_plot <- renderPlot(
    {
      ...
      ggplot(grafico[1:10, ], aes(x = `Combined.Score`, y = `Term`, label = round(`Combined.Score`))) +
        ...
        geom_point(size = 8, color = "#f2545b") +
